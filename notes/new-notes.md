...
## RS触发器的不定态
![](./notes.dia/RS-trigger-unstable-understand.png)
...
![](./notes.dia/io-clock-architecture.png)
    1. common单向clk
...
## DTC SDC
## 竞争冒险/亚稳态
    产生电路误操作的原因主要有：竞争与冒险（产生毛刺、非预期的值），
    建立时间和保持时间违规（亚稳态等）等
### 竞争冒险
    1. 与门的俩个输入信号，同时向相反方向翻转，叫做这俩信号的竞争。
    如果在到达与门的前一刻，一个先完成翻转，另一个后完成，这种就产生了冒险，
    如果同时完成翻转，就不会产生冒险。冒险就会造成尖峰脉冲-毛刺。
    2. 竞争和险象可能导致毛刺，防止毛刺发生的策略:
        2.1 输出信号使用寄存器消除毛刺
        2.2 采用格雷码消除毛刺
        2.3 卡诺图消除毛刺（添加冗余逻辑）
### 亚稳态
    1. 亚稳态是指触发器无法在某个规定的时间内达到一个可确认的状态
    2. 如果触发器的建立时间（setup time） /保持时间（ hold time）不满足，就可能产生亚稳态
    3. 只要系统中有异步元件，亚稳态的发生就不可避免,亚稳态不能从根本上消除，
    但可以通过采取一些措施使其对电路造成的影响降低
    4. 经常使用的方法是使用两级触发器同步来消除亚稳态，原因是因为增加寄存器进行同步，
    只是不断降低亚稳态出现的频率，并不会消除亚稳态
### 总结
    1. 竞争冒险出现在组合逻辑电路中，由于多个信号到达门器件的时间不一致导致；
    2. 亚稳态是数据信号与clock信号之间的setup/hold时间不合规造成；
    3. 组合逻辑电路出现的是竞争冒险问题；
    时序逻辑电路出现的是亚稳态问题；
    4. 亚稳态经常出现在异步电路中
## 复位信号的意义
...
## 串并行/同异步
### 串行
    信号传输按单bit进行，俩个事物的运行有先后顺序，先后之间不一定有固定的时间差。
### 并行
    俩个事物可以各自独立运行，彼此不依赖。比如verilog中的多个always块就是独立运行的，
    我们就说这俩个always块是并行运行的。
### 同步
    俩个信号有固定的相位关系，或者俩个事物的运行，在时间上有固定的差值
### 异步
    俩个信号的相位关系不固定，或者俩个事物的运行，在时间上没有固定的差值
### 总结
    并行不一定同步
    串行不一定同步
## verilog语法实例学习(8)
